...
現在の実装は、本当に電撃が流れる装置ではなく、LEDテストデバイスをESP32で操作する前提です。README上の既存コンセプトや画像はそのまま残しつつ、実装側ではBLEで `RealShockLED` を自動探索して接続し、`event <kind> <level> <duration_ms> <id>` をESP32へ送ります。USBシリアル送信もデバッグ用に残しています。

ボタン割り当ては `A=GPIO23` が威力アップ、`B=GPIO22` がモード変更、`C=GPIO19` が威力ダウンです。ESP32は起動時にAを1回押してレベル0へ戻し、Bを2回押してモード3にします。レベル0のまま13秒以上経っていた場合は、次の出力前にCを1回押して電源OFF相当の `-1` として扱ってから、必要レベルまでAを押します。
