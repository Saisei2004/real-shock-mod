...
**RE:AL SHOCK MOD** is a local Windows system that watches **BIOHAZARD requiem / Resident Evil Requiem**, reads the player's HP state through a REFramework Lua bridge, reads the player's real biometric signal from a Bluetooth heart-rate sensor, and emits a prioritized command to an ESP32 on the same network.

The Japanese README is the main documentation: [README.md](README.md)
...
![RE:AL SHOCK MOD dashboard](docs/images/ui-normal.png)

The UI screenshots in this repository are captured from the actual app with `?demo=` pseudo-data. They are not AI-generated mock images.
...
```mermaid
flowchart LR
    A["BIOHAZARD requiem"] -->|"HP / damage / death JSON"| B["REFramework Lua Bridge"]
